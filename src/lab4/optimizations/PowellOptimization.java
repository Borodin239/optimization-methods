...
public class PowellOptimization implements Optimization {
    @Override
    public Result solve(final double[] values, final FunctionInfo function, final double epsilon) {
        ...
        while (true) {
            ...
            double[] deltaXk  = baseOperations.vectorSum(deltaX, baseOperations.multiplyMatrixOnVector(G, deltaW));
            G = baseOperations.matrixSub(G, baseOperations.multiplyMatrixOnNumber(
                    baseOperations.multiplyColumnOnRow(deltaXk, deltaXk),
                    1 / baseOperations.scalar(deltaW, deltaXk)
            ));
